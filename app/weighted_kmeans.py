...
def update_sites(k, sites, X, W):
    ...
    for i in range(X.shape[0]):
        ...
        min_site = r.permutation(np.where(distance == np.min(distance))[0])[
            0
        ]  # Randomly break ties
...
def weighted_kmeans(k, n, d, X, W, iters=1):
    ...
    return clusterings[
        np.min(list(clusterings.keys()))
    ]  # Return the best clustering based on distance


def hierarchical_kmeans(n, d, X, W, k_plus):
    """Performs hierarchical k-means clustering recursively."""
    queue, pos, classif, clusterings = (
        deque([np.array(range(n))]),
        np.arange(n),
        0,
        np.zeros(n),
    )
    ...
    while queue:
        ...
        clusters = get_clusters(
            sub_X, pos[c], weighted_kmeans(2, sub_n, d, sub_X, sub_W, 1), 2
        )
