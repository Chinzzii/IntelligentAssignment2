...
def team_swap(teams, users):
    ...
    for team in teams:
        if team.users_to_pawn():  # Check if team has users to pawn
            auct_users.extend(team.users_to_pawn())  # Add users to auction list
            auct_teams.append(team)  # Add team to auction list
    top_trading_cycles(auct_teams, auct_users)  # Perform top trading cycles on the selected teams and users
