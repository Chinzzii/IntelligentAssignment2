...
class TeamTestCase(unittest.TestCase):
    ...
    def test_merge_teams(self):
        ...
        response = self.app.post('/merge_teams', data=json.dumps(data), content_type='application/json')
    ...
    def test_swap_team_members(self):
        ...
        response = self.app.post('/swap_team_members', data=json.dumps(data), content_type='application/json')
    ...
    def test_invalid_input(self):
        ...
        response = self.app.post('/merge_teams', data=json.dumps(invalid_data), content_type='application/json')
        self.assertEqual(response.status_code, 400)

        # Missing teams field
        invalid_data = {"users": [{"ranks": [1, 0, 2, 3], "pid": 1023}]}
        response = self.app.post('/swap_team_members', data=json.dumps(invalid_data), content_type='application/json')
        self.assertEqual(response.status_code, 400)
